...
function localLoad(): JobBlocker[] {
  ...
  try { const raw = window.localStorage.getItem(LS_KEY); return raw ? (JSON.parse(raw) as JobBlocker[]) : []; }
  catch { return []; }
}
function localSave(blockers: JobBlocker[]) {
  if (typeof window === "undefined") return;
  try { window.localStorage.setItem(LS_KEY, JSON.stringify(blockers)); } catch { /* silent */ }
}
...
export function JobBlockersProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    if (!isSb) { setBlockers(localLoad()); setLoading(false); return; }
    try {
      const { data, error } = await getSupabase().from(TABLE).select("*").order("raised_at", { ascending: false });
      if (error) throw error;
      setBlockers((data ?? []).map(rowToBlocker)); setError(null);
    } catch (e) { setError(formatError(e)); setBlockers(localLoad()); }
    finally { setLoading(false); }
  }, [isSb]);

  useEffect(() => { void refresh(); }, [refresh]);
  useEffect(() => { if (!loading && !isSb) localSave(blockers); }, [blockers, loading, isSb]);

  const addBlocker = useCallback(async (b: NewJobBlocker) => {
    const blocker: JobBlocker = { ...b, id: newId(), raisedAt: new Date().toISOString(), resolvedAt: null };
    setBlockers((prev) => [blocker, ...prev]);
    if (isSb) {
      try { const { error } = await getSupabase().from(TABLE).insert(blockerToRow(blocker)); if (error) throw error; setError(null); }
      catch (e) { setError(formatError(e)); setBlockers((prev) => prev.filter((x) => x.id !== blocker.id)); }
    }
    return blocker.id;
  }, [isSb]);
  ...
  const resolveBlocker = useCallback((id: string) => {
    ...
    if (isSb) {
      void getSupabase().from(TABLE).update({ resolved_at: resolvedAt, updated_at: resolvedAt }).eq("id", id)
        .then(({ error }) => { if (error) setError(formatError(error)); });
    }
  }, [isSb]);

  const reopenBlocker = useCallback((id: string) => {
    const updatedAt = new Date().toISOString();
    setBlockers((prev) => prev.map((b) => (b.id === id ? { ...b, resolvedAt: null } : b)));
    if (isSb) {
      void getSupabase().from(TABLE).update({ resolved_at: null, updated_at: updatedAt }).eq("id", id)
        .then(({ error }) => { if (error) setError(formatError(error)); });
    }
  }, [isSb]);
  ...
  return (
    <JobBlockersContext.Provider value={{ blockers, loading, error, activeByJob, activeForJob, addBlocker, resolveBlocker, reopenBlocker, refresh }}>
      {children}
