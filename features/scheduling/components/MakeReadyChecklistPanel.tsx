...
import { useMakeReady } from "../lib/makeReadyStore";
  ...
  return (
    ...
    >
      ...
      {isAuto ? (
        ...
        <span
          aria-label={
            item.checked ? `${item.label} — auto-ticked` : `${item.label} — waiting for signal`
          }
          className={cn(
      ...
      <div className="flex-1 min-w-0">
        <span className={cn("text-sm", resolved ? "text-text-primary" : "text-text-secondary")}>
          {item.label}
        </span>
        {isAuto && (
        ...
        {item.overridden && <span className="ml-1.5 text-xs text-amber-600">overridden</span>}
      </div>
    ...
    >
      ...
      {!summary.ready && (
        ...
        >
          ...
          {summary.total - summary.checkedCount !== 1 ? "s" : ""} outstanding. You can still proceed
          (soft gate).
        </div>
      )}
...
export function MakeReadyChecklistPanel({
  ...
  // All make-ready persistence lives behind the store seam; this component is
  // render-of-state. `itemsByPhase` holds the raw items (standard items merged
  // with saved state, before auto-signals are applied at render time).
  const { itemsByPhase, loading, busyId, error, updateItem } = useMakeReady(jobId);
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xs uppercase tracking-[0.06em] text-text-tertiary">Make-Ready Gate</h3>
        {allReady ? (
          <span
          ...
          <span className="text-xs text-text-tertiary">Soft gate — warnings only (ADR 0013)</span>
        )}
      </div>
      ...
      <p className="mb-4 text-xs text-text-tertiary">
        Before committing to start a phase, confirm all prerequisites are in place. Auto-ticked
        items are derived from existing job signals. Unchecked items warn but never block progress.
      </p>
