...
// ─── Data access ──────────────────────────────────────────────────────────────
//
// ADR 0022: reads + writes both live on the generalized `share_tokens` registry
// (capability_type=schedule). The legacy `schedule_share_links` mirror was
// retired in #269 after a row-for-row prod verify.
...
/**
 * Insert a new share link into share_tokens (the read path). Returns true only
 * when the share_tokens write — the one the portal reads — succeeds.
 */
...
/** Stamp `revoked_at` on a share link in share_tokens (the read path). */
