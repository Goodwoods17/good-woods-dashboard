...
import { encryptToken, decryptToken, tokenEncryptionConfigured } from "./qboTokenCrypto";

const SECRET = "test-secret-key-do-not-use-in-prod-0000000000";

describe("qboTokenCrypto", () => {
  it("round-trips a refresh token through encrypt → decrypt", () => {
    const plain = "AB11700000000refresh-token-abc.def_GHI-jkl";
  ...
  it("rejects a tampered ciphertext (GCM auth tag fails)", () => {
    ...
    // Flip the tail of the ciphertext segment.
