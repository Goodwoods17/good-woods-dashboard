...
/**
 * Symmetric encryption for the QuickBooks Online OAuth **refresh token** (and
 * the short-lived access token) stored at rest (QBO S1, issue #147). The refresh
 * token is a long-lived credential that QBO ROTATES roughly every 24h — it MUST
 * NOT sit in the database in plaintext. We encrypt it server-side with
 * AES-256-GCM keyed by the `QBO_TOKEN_ENC_KEY` env secret (never shipped to the
 * client) and store only the ciphertext blob in `quickbooks_connection`.
 *
 ...
 * callback + refresh paths), never bundled into the client. This deliberately
 * mirrors `features/scheduling/lib/googleTokenCrypto.ts` (S23) so the two token
 * stores stay identical in behaviour and review surface.
 */
